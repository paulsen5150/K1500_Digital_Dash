...
# VERSION 0.3.1 Removing MQTT and adding support for ESP32 data communication 2026.05.06
# kate: indent-pasted-text false; indent-width 4;
# import pygame module
import pygame
from constants31 import *
import functions31 as functions31_module
from functions31 import *
...
SERIAL_PORT = os.getenv("KITT_SERIAL_PORT", "/dev/ttyACM0")
SERIAL_BAUD = int(os.getenv("KITT_SERIAL_BAUD", "115200"))
SERIAL_TIMEOUT = float(os.getenv("KITT_SERIAL_TIMEOUT", "0.05"))

try:
    ser = serial.Serial(SERIAL_PORT, SERIAL_BAUD, timeout=SERIAL_TIMEOUT)
except SerialException as exc:
    print(f"Serial unavailable on {SERIAL_PORT}: {exc}")
    ser = None
...
def update_dash_state(dash_state):
    global illumination_state, foglight_state, high4_state, highbeam_state
    global leftturn_state, rightturn_state, brakewarn_state, oillight_state
    global alt_state, mil_state, fuel_status, oil_status, coolant_status
    global tach_status, speed_status, odometer_status

    illumination_state = as_bool(dash_state.get("illumination"), illumination_state)
    foglight_state = as_bool(dash_state.get("foglight"), foglight_state)
    high4_state = as_bool(dash_state.get("high4"), high4_state)
    highbeam_state = as_bool(dash_state.get("highbeam"), highbeam_state)
    leftturn_state = as_bool(dash_state.get("left_signal"), leftturn_state)
    rightturn_state = as_bool(dash_state.get("right_signal"), rightturn_state)
    brakewarn_state = as_bool(dash_state.get("brakewarn"), brakewarn_state)
    oillight_state = as_bool(dash_state.get("oilwarn"), oillight_state)
    alt_state = as_bool(dash_state.get("altwarn"), alt_state)
    mil_state = as_bool(dash_state.get("milwarn"), mil_state)

    fuel_status = as_number(dash_state.get("fuel_percent"), fuel_status)
    oil_status = as_number(dash_state.get("oil_psi"), oil_status)
    coolant_status = as_number(dash_state.get("ECT"), coolant_status)
    tach_status = as_number(dash_state.get("tach"), tach_status)
    speed_status = as_number(dash_state.get("speed"), speed_status)
    odometer_status = as_number(dash_state.get("odometer"), globals().get("odometer_status", 0))


def read_serial_dash_state():
    if ser is None:
        return
    ...
    update_dash_state(dash_state)


def set_display_mode(flags=0):
    global gauge_window
    gauge_window = pygame.display.set_mode((window_width, window_height), flags)
    functions31_module.gauge_window = gauge_window
    return gauge_window
...
while running:
	...
	# READ DATA FROM KITT interface board
	read_serial_dash_state()
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYDOWN:
			...
			if event.key == pygame.K_F11:
				...
				if fullscreen:
					set_display_mode(pygame.FULLSCREEN)
